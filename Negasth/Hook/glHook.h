			...
			for (EntityPlayer& target : Negasth::cache->playerEntities.toVector<EntityPlayer>(renderEnv))
			{
				...
				if (playerTeam.isValid()) {
					...
					if (strstr(teamName, "RED")) {
						...
						DrawHealth(curHealth, maxHealth, minx, miny, minz, maxx, maxy, maxz, 255, 182, 193);
						DrawBoxOutline(minx, miny, minz, maxx, maxy, maxz, 255, 182, 193);
					}
				}
				else {
					DrawHealth(curHealth, maxHealth, minx, miny, minz, maxx, maxy, maxz, 255, 182, 193);
				...
				//renderEnv->DeleteLocalRef(target.getInstance());
				renderEnv->DeleteLocalRef(targetBB.getInstance());
				renderEnv->DeleteLocalRef(playerTeam.getInstance());
			}
		}
