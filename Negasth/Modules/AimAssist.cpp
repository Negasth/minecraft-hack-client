...
#include "../net/Minecraft/item/ItemSword/ItemSword.h"
void Negasth::Modules::AimAssist::run() {
	if (!(GetKeyState(VK_LBUTTON) & 0x8000)|| cache->theMinecraft.getCurrentScreen().isValid() || Negasth::p_env->IsInstanceOf(cache->thePlayer.getHeldItem().getItem().getInstance(), ItemSword::ItemSwordClass.getJclass(Negasth::p_env)) != JNI_TRUE)
	{
	...
	for (EntityPlayer& target : Negasth::cache->playerEntities.toVector<EntityPlayer>())
	{
		...
		if (!target.isValid() || target.isEqualto(cache->thePlayer) || !cache->thePlayer.canAttackPlayer(target) ) /*|| target.getTicksExisted() < 10*/
			continue;
		...
		Negasth::p_env->DeleteLocalRef(myTeam.getInstance());
		Negasth::p_env->DeleteLocalRef(playerTeam.getInstance());
		Negasth::p_env->DeleteLocalRef(targetBB.getInstance());
