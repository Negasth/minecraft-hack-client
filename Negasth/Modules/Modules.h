...
namespace Negasth {
	namespace Modules {
		...
		class CheckPackets :public IModule {
		public:
			CheckPackets() :IModule("CheckPackets") {};
			void run()override;

			inline static std::list<std::chrono::seconds> packetsList;
			const inline static int maxPackets = 200;
			inline static int nowPackets = 0;
			inline static std::chrono::seconds lastTime;
		private:
			std::chrono::seconds coolDownSecond = std::chrono::seconds(300);
		};

		class HitBoxes :public IModule {
		public:
			HitBoxes() :IModule("HitBoxes") {};
			void run()override;
		private:
			float x_expand = 0.2f;
			float y_expand = 0.2f;
		...
		class Refill :public IModule {
		public:
			Refill() :IModule("Refill") {};
			void run()override;
		private:
			Item preItem;
			int preIndex = -1;
		...
		class FastBreak :public IModule {
		public:
			FastBreak() :IModule("FastBreak") {};
			void run()override;
		private:
			float lastBlockDamageMP;
		};

		class AutoArmor :public IModule {
		public:
			AutoArmor() :IModule("AutoArmor") {};
			void run() override;
		};
		...
		inline Category categories[] = {
			Category::create<AimAssist,AutoClick/*,KillAura*/,HitBoxes>("Combat"),
			Category::create<FastPlace,FastBreak,AutoArmor>("Player"),
			Category::create<Refill,CheckPackets>("Other"),
