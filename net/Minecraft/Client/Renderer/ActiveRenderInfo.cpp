...
GLfloat* ActiveRenderInfo::Get_MODELVIEW(JNIEnv* env)
{
	...
	jobject OMODELVIEW =  env->GetStaticObjectField(ActiveRenderInfoClass.getJclass(env), ActiveRenderInfoClass.getFieldID("MODELVIEW"));
...
{
	jobject OPROJECTION = env->GetStaticObjectField(ActiveRenderInfoClass.getJclass(env), ActiveRenderInfoClass.getFieldID("PROJECTION"));
	jclass CFloatBuffer = env->FindClass("java/nio/FloatBuffer");
	jmethodID MIDget = env->GetMethodID(CFloatBuffer, "get", "(I)F");
	for (int i = 0; i < 16; i++)
	{
