...
C03PacketPlayer C03PacketPlayer::newObject(bool isOnGround, JNIEnv* env)
{
	...
	return C03PacketPlayer(env->NewObject(C03PacketPlayerClass.getJclass(env), C03PacketPlayerClass.getMethodID("<init>"), (isOnGround ? JNI_TRUE : JNI_FALSE)), env);
}

Negasth::Maths::Vector3d C03PacketPlayer::getXYZ()
{
	if (!instance)
...
void C03PacketPlayer::setXYZ(const Negasth::Maths::Vector3d& vector)
{
	if (!instance)
		return;
...
Negasth::Maths::Vector2d C03PacketPlayer::getYawPitch()
{
	if (!instance)
...
void C03PacketPlayer::setYawPitch(Negasth::Maths::Vector2d yawPitch)
{
	if (!instance)
		return;
